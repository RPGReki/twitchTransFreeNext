...
async def save(message,translation,dlang):  # en:Save the translations   ja:翻訳を保存する
    values={"message": message,"translation": translation, "dlang": dlang}
    db.execute(f'INSERT INTO {table_name} (MESSAGE,DLANG,TRANSLATION) VALUES (:message, :dlang, :translation)', values)
    db.commit()
...
async def get(message,dlang):  # en:Get the translations    ja:翻訳を入手する
    ...
    values={"message": message, "dlang": dlang}
    return db.execute(f"SELECT TRANSLATION FROM {table_name} WHERE MESSAGE=:message AND DLANG=:dlang", values).fetchone()
